...
{
    [CustomEditor(typeof(SceneTestDriver))]
    public class SceneTestDriverEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            // 현재 인스펙터에 그려지고 있는 TestDriver 타겟 가져오기
            SceneTestDriver driver = (SceneTestDriver)target;
            ...
            if (driver != null)
            {
                ...
                if (components.Length > 2)
                {
                    ...
                    EditorGUILayout.HelpBox(
                        $"⚠️ [주의] 이 GameObject에 {nameof(SceneTestDriver)} 외의 다른 컴포넌트가 감지되었습니다!\n\n" +
                        $"{nameof(SceneTestDriver)}는 단독 씬 테스트 환경을 구축한 후, 혹은 일반 씬 흐름일 경우 " +
                        $"게임 시작 직후(Awake/Start) 스스로 자폭(Destroy)합니다.\n\n" +
                        $"따라서 여기에 함께 부착된 다른 컴포넌트들도 모두 강제 파괴되어 " +
                        $"정상적으로 동작하지 않습니다. {nameof(SceneTestDriver)}는 반드시 단독 객체로 사용해 주세요.",
                        MessageType.Warning);
